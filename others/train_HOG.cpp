...
void sample_neg( const vector< Mat > & full_neg_lst, vector< Mat > & neg_lst, const Size & size )
{
    ...
    for ( size_t i = 0; i < full_neg_lst.size(); i++ )
    {
        ...
        else if(full_neg_lst[i].cols == box.width && full_neg_lst[i].rows == box.height)
        {
        	box.x = full_neg_lst[i].cols;
            box.y = full_neg_lst[i].rows;
...
int main( int argc, char** argv )
{
    ...
    clog << "...[done] , neg_lst.size() = " << neg_lst.size() << endl;
